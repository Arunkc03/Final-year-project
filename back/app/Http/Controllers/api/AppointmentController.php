...
class AppointmentController extends Controller
{
    ...
    public function doctorAppointments(Request $request)
    {
        ...
        // Get doctor's appointments - only show paid appointments
        $appointments = Appointment::where('doctor_id', $doctor->id)
            ->where('payment_status', 'completed') // Only show paid appointments
    ...
    public function acceptAppointment(Request $request, $id)
    {
        ...
        try {
            ...
            // Notify patient about acceptance (email notification)
            $patient = $appointment->user;
            $patient->notify(new \App\Notifications\AppointmentAccepted($appointment));
            
            // Also store in-app notification via NotificationService
            \App\Services\NotificationService::sendAppointmentNotification($appointment, 'confirmed');
    ...
    public function rejectAppointment(Request $request, $id)
    {
        ...
        try {
            ...
            // Notify patient about rejection (email notification)
            $patient = $appointment->user;
            $patient->notify(new \App\Notifications\AppointmentRejected($appointment));
            
            // Also store in-app notification via NotificationService
            \App\Services\NotificationService::sendAppointmentNotification($appointment, 'cancelled');
